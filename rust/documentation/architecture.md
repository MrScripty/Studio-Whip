...
2.  **Rendering Engine (`rendering/render_engine.rs`)**
    *   Role: Orchestrates the custom Vulkan rendering pipeline per frame. Manages `BufferManager` (for shapes/cursors) and `TextRenderer` (for text). Manages sync objects (semaphores, fence). Handles resize via `ResizeHandler`. Calls `BufferManager` (shapes/cursors) and `TextRenderer` (text) to prepare draw data, then calls `command_buffers` to record draw calls. **Resets the active command buffer each frame.** Accessed via `RendererResource`.
...
## Data Flow (Updated for Command Buffer/Renderer Locking Refactor)
...
2.  Bevy `Startup` schedule runs (ordered by `CoreSet`, `InteractionSet`):
    ...
    *   `create_renderer_system` creates `RendererResource`. **This involves creating the command pool (stored in `VulkanContext`), then swapchain, then framebuffers (which allocate command buffers from the pool via `swapchain.rs` functions).**
...
4.  Bevy `Last` schedule runs:
    ...
    *   `Renderer::render`:
        ...
        *   Calls `BufferManager::prepare_frame_resources`.
    ...
    *   `cleanup_trigger_system` (Core Plugin) runs on `AppExit`, cleaning up **`TextBufferCache` components**, then framework resources (`Renderer` (which cleans its `BufferManager`, `TextRenderer`, and **command pool from `VulkanContext`**), `TextRenderingResources`, `GlobalProjectionUboResource`, `GlyphAtlas`, `VulkanContext` (which cleans remaining resources)).

## Key Interactions (Updated for Command Buffer/Renderer Locking Refactor)
...
- **Core Plugin (Cleanup)**: `cleanup_trigger_system` queries/cleans up `TextBufferCache`, then cleans up `Renderer` (and its internal `BufferManager`, `TextRenderer`, **and its managed command pool from `VulkanContext`**), `TextRenderingResources`, `GlobalProjectionUboResource`, `GlyphAtlas`, `VulkanContext`.
...
- **TextRenderer <-> TextLayoutInfo**: `TextRenderer::prepare_text_draws` consumes `TextLayoutInfo` (containing `TextLayoutOutput`, `GlobalTransform`, `Visibility`) to generate text draw data and manage its internal `TextRenderData` cache.
- **Renderer <-> ResizeHandler**: `Renderer::resize_renderer` calls `ResizeHandler::resize`.
- **ResizeHandler <-> Swapchain**: `ResizeHandler::resize` calls `swapchain.rs` functions: `cleanup_swapchain_resources` (which frees old command buffers), then `create_swapchain`, then `create_framebuffers` (which allocates new command buffers).
- **Swapchain Module <-> VulkanContext**: `swapchain.rs` functions (`create_swapchain`, `create_framebuffers`, `cleanup_swapchain_resources`) directly read from and write to `VulkanContext` to manage swapchain images, views, framebuffers, render pass, and **command buffers**.
...
- **VulkanContext <-> Vulkan Components**: Provides core Vulkan resources, including depth buffer, command pool, command buffers, and debug messenger.
- **Build Script -> Target Directory**: Compiles shaders, copies assets.

## Current Capabilities (Updated for Command Buffer/Renderer Locking Refactor)
...
- **Robust Shutdown**: Application exit sequence correctly cleans up **`TextBufferCache` components**, then `Renderer` (which cleans its internal `BufferManager`, `TextRenderer`, **and its managed command pool from `VulkanContext`**), and then shared Vulkan resources via `cleanup_trigger_system`.
...
- **Rendering Optimization**: Implement resource removal for despawned entities in `BufferManager` (using `RemovedComponents`). Optimize text vertex buffer updates in `TextRenderer` (e.g., smarter resizing, partial updates).
