...
/*
 * Section: Application
 * Purpose: Defines the top-level App struct and its initialization; orchestrates the application lifecycle.
 * Passes to:
 *   - Window Management: ActiveEventLoop (via resumed) for window creation and event handling
 * Receives from:
 *   - Window Management: Arc<Window> (stored in self.window)
 *   - Vulkan Core: Vulkan resources (stored in self fields like entry, instance, device)
 *   - Renderer: Rendering resources (stored in self fields like swapchain, pipeline)
 */
struct App {
...
impl App {
    ...
    /*
     * Section: Vulkan Core
     * Purpose: Initializes and cleans up core Vulkan resources (entry, instance, device, queue, allocator).
     * Passes to:
     *   - Renderer: Entry, Instance, Device, Queue, Arc<Allocator> (via self fields)
     * Receives from:
     *   - Window Management: Arc<Window> for surface creation
     */
    fn setup_vulkan(&mut self, window: Arc<Window>) {
    ...
    /*
     * Section: Renderer
     * Purpose: Manages rendering setup (swapchain, pipeline, etc.) and drawing the frame; cleans up rendering resources.
     * Passes to:
     *   - Application: Updated self fields (e.g., swapchain, pipeline, current_image)
     * Receives from:
     *   - Window Management: vk::Extent2D for swapchain setup
     *   - Vulkan Core: Entry, Instance, Device, Queue, Arc<Allocator> (via self fields)
     */
    fn setup_renderer(&mut self, extent: vk::Extent2D) {
        let instance = self.instance.as_ref().unwrap();
        let device = self.device.as_ref().unwrap();
        let surface = self.surface.unwrap();
        let surface_loader = self.surface_loader.as_ref().unwrap();
        let physical_device = unsafe { instance.enumerate_physical_devices().unwrap() }[0]; // Simplified for now
        ...
        let (vertex_buffer, vertex_allocation) = {
            ...
            let (buffer, mut allocation) = unsafe {
                self.allocator.as_ref().unwrap().create_buffer(&buffer_info, &allocation_info)
            }
            .unwrap();
            let data_ptr = unsafe { self.allocator.as_ref().unwrap().map_memory(&mut allocation) }
                .unwrap()
                .cast::<Vertex>();
            unsafe { data_ptr.copy_from_nonoverlapping(vertices.as_ptr(), vertices.len()) };
            unsafe { self.allocator.as_ref().unwrap().unmap_memory(&mut allocation) };
            (buffer, allocation)
        };
    ...
    fn cleanup_renderer(&mut self) {
        let device = self.device.as_ref().unwrap(); // Borrow device, cleaned up by Vulkan Core
        ...
        unsafe {
            ...
            self.allocator
                .as_ref()
                .unwrap()
                .destroy_buffer(self.vertex_buffer.take().unwrap(), &mut self.vertex_allocation.take().unwrap());
 ...
 * Purpose: Creates and manages the winit window, dispatching events to the renderer.
 * Passes to:
 *   - Vulkan Core: Arc<Window> for surface creation
 *   - Renderer: vk::Extent2D for swapchain setup
 * Receives from:
 *   - Application: ActiveEventLoop for window creation and event handling
 */
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        if self.window.is_none() {
            ...
            self.setup_vulkan(window);
            self.setup_renderer(extent);
        }
    }
    ...
    fn window_event(&mut self, event_loop: &ActiveEventLoop, _id: WindowId, event: WindowEvent) {
        match event {
            ...
            WindowEvent::RedrawRequested => {
                self.render(); // Delegate rendering to the Renderer section
                if let Some(window) = &self.window {
                    window.request_redraw(); // Window management triggers redraw
                }
            }
            _ => (),
        }
...
/*
 * Section: Main
 * Purpose: Sets up the event loop and runs the application.
 * Passes to:
 *   - Application: EventLoop for App initialization and running
 * Receives from:
 *   - Application: None
 */
#[allow(deprecated)]
