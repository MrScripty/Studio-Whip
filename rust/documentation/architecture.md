...
2.  **Rendering Engine (`rendering/`)**
    *   Role: Orchestrates the custom Vulkan rendering pipeline per frame. Manages `BufferManager` (for shapes), `GlyphAtlas` (for text), descriptor pool/layout handles, and Vulkan sync objects (fences, semaphores). Calls `BufferManager` (shapes) and potentially text rendering logic to prepare resources based on ECS data and `command_buffers` to record draw calls. Handles swapchain recreation on resize, calculating projection matrix based on **logical window size with Y-flip** for Vulkan NDC. Accessed via `RendererResource`.
...
5.  **Text Handling (`rendering/font_server.rs`, `rendering/glyph_atlas.rs`, `components/text_data.rs`, `components/text_layout.rs`)**
    *   Role: Manages font loading, text layout/shaping, glyph caching/rasterization/upload, and prepares text data for rendering.
    *   Key Modules: `font_server.rs` (loads fonts via `fontdb`, provides `cosmic_text::FontSystem`), `glyph_atlas.rs` (manages Vulkan texture atlas, performs packing, rasterization via `swash`, and uploads glyphs), `text_data.rs` (`Text` component), `text_layout.rs` (`TextLayoutOutput` component). Systems in `plugins/core.rs` orchestrate layout.
...
## Data Flow (Post Plugin Refactor & Text Foundation Update)
...
2.  Bevy `Startup` schedule runs:
    *   `GuiFrameworkCorePlugin` systems (`CoreSet::SetupVulkan`, `CoreSet::CreateRenderer`, `CoreSet::CreateGlyphAtlas`, `CoreSet::CreateFontServer`, `CoreSet::CreateSwashCache`) initialize Vulkan, create the `RendererResource`, `GlyphAtlasResource`, `FontServerResource`, `SwashCacheResource`. `CreateRenderer` uses `PipelineManager` to set up layouts/pool.
...
3.  Bevy `Update` schedule runs:
    ...
    *   `GuiFrameworkCorePlugin` system `text_layout_system` (`CoreSet::TextLayout`, ordered `.after(CoreSet::HandleResize)`) queries `Text` components, uses `FontServerResource`, `SwashCacheResource` to get rasterized glyph data (`swash::Image`), calls `GlyphAtlas::add_glyph` (passing `VulkanContextResource`) to pack/upload glyphs to the atlas texture, and inserts/updates `TextLayoutOutput` components.
4.  Bevy `Last` schedule runs:
    *   `GuiFrameworkCorePlugin` system `rendering_system` (`CoreSet::Render`, ordered `.after(CoreSet::TextLayout)`, runs if `AppExit` not sent) queries ECS for shapes (`RenderCommandData`) and text (`TextLayoutOutput`), collects render data (currently only shapes), and calls `Renderer::render`.
...
## Key Interactions (Post Plugin Refactor & Text Foundation Update)
...
- **Core Plugin (Text Layout)**: `text_layout_system` reads `Text`, `Transform`; reads/writes `FontServerResource`, `SwashCacheResource`; reads `VulkanContextResource`; reads/writes `GlyphAtlasResource` (via `add_glyph`); writes `TextLayoutOutput`.
...
- **Renderer <-> GlyphAtlas**: Renderer will need access to the glyph atlas texture/sampler for text rendering (future).
...
## Current Capabilities (Post Plugin Refactor & Text Foundation Update)
...
- **Bevy ECS**: Defines and uses custom components (`ShapeData`, `Visibility`, `Interaction`, `BackgroundQuad`, `Text`, `TextLayoutOutput`, `TextVertex`) and `bevy_transform::components::Transform`.
...
- **ECS-Driven Resource Management**: `BufferManager` creates/updates Vulkan buffers/descriptors for **shapes**. `GlyphAtlas` manages Vulkan texture for glyphs, including packing, rasterization (via `swash`), and upload.
- **Resource Caching**: `BufferManager` caches Vulkan `Pipeline` and `ShaderModule` resources (for shapes). `GlyphAtlas` caches glyph locations/UVs.
- **Rendering Path**: Data flows from ECS (`rendering_system`) through `Renderer` to `BufferManager` (shape prep) and `command_buffers` (shape draw recording). Synchronization corrected using fences. **Projection matrix uses logical window size and Y-flip.** (Text rendering path setup initiated, but rendering not implemented).
...
- **Text Foundation**:
    ...
    - Text shaders (`text.vert`, `text.frag`) created.
    - Text-specific Vulkan layouts created.
...
## Future Extensions
- **Text Handling (Task 8+)**: Implement text rendering pipeline (vertex generation, buffer management, draw calls), text editing (`yrs`), context menus (Task 9).
- **Rendering Optimization**: Implement resource removal for despawned entities (using `RemovedComponents`). Optimize text vertex buffer management.
...
- Text shaders (`text.vert`, `text.frag`) created. `text.vert` uses Set 0 (Binding 0: projection). `text.frag` uses Set 1 (Binding 0: glyph atlas sampler).
...
- **Known Issues**: `BufferManager` lacks resource removal for despawned entities and only handles shapes. Hit detection Z-sorting needs review. `vulkan_setup` still uses `winit::window::Window` directly. **Text rendering pipeline not implemented.**
