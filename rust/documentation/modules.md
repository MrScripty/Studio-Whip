...
### Current State (Post Text Atlas Implementation)
...
- **Rendering Status**: Rendering triggered by `rendering_system` (in `GuiFrameworkCorePlugin`). `BufferManager` creates/updates Vulkan resources for **shapes only**. `GlyphAtlas` manages Vulkan texture for glyphs, including packing and upload. `command_buffers` records draw calls **for shapes only**. Synchronization and resize handling are corrected. **Visual output functional for shapes.** Text rendering pipeline setup initiated (shaders, layouts) but not implemented. **Optimizations needed: resource removal for despawned entities.**
- **Text Handling**: Foundation laid with `Text` component, `FontServer` (loading system fonts), `GlyphAtlas` (Vulkan texture setup, packing, rasterization via `swash`, upload), `SwashCache` resource, and `text_layout_system` (CPU-side layout using `cosmic-text`, calls `add_glyph`). Text shaders and Vulkan layouts created. Glyph rendering pipeline is pending.
...
### `src/lib.rs`
- **Purpose**: Defines shared types (`Vertex`, `TextVertex`, `RenderCommandData`, `PreparedDrawData`) and shared framework resources (`VulkanContextResource`, `RendererResource`, `HotkeyResource`, `GlyphAtlasResource`, `FontServerResource`, `SwashCacheResource`).
- **Key Structs**:
...
- **Key Structs**: `VulkanContext { ... current_swap_extent: vk::Extent2D, ... shape_pipeline_layout: Option<vk::PipelineLayout>, text_pipeline_layout: Option<vk::PipelineLayout>, ... allocator: Option<Arc<Allocator>>, ... command_pool: Option<vk::CommandPool>, ... }`. (**Updated fields**)
...
- **Purpose**: Plugin for core Vulkan setup, rendering, text foundation (setup, layout, atlas upload), resize handling, and cleanup.
...
- **Notes**: Adds systems to `Startup`, `Update`, `Last` schedules. Configures `CoreSet` ordering. Registers core types (`ShapeData`, `Visibility`, `Vertex`, `Text`, `FontId`, `TextAlignment`, `Color`, `Vec2`, `IVec2`). Inserts `RendererResource`, `GlyphAtlasResource`, `FontServerResource`, `SwashCacheResource`. `text_layout_system` performs CPU layout and triggers glyph upload via `GlyphAtlas::add_glyph`. `rendering_system` prepares shape data (text data pending). `cleanup_trigger_system` cleans Vulkan resources including `GlyphAtlas`.
...
- **Notes**: Called by `render_engine.rs`. Uses `platform.current_swap_extent`. Uses `shape_pipeline_layout`. **Needs modification for text rendering (binding text pipeline, text vertex buffers, text descriptor sets).**
...
- **Key Structs**: `Renderer { buffer_manager: BufferManager, descriptor_pool: vk::DescriptorPool, descriptor_set_layout: vk::DescriptorSetLayout, text_descriptor_set_layout: vk::DescriptorSetLayout }`. (**Updated fields**)
- **Key Methods**: `new(...) -> Self`, `resize_renderer(...) -> ()`, `render(..., shape_commands: &[RenderCommandData] /*, text_commands: ... */) -> ()`, `cleanup(...) -> ()`.
- **Notes**: Managed via `RendererResource`. Called by `rendering_system` (core plugin). `new` initializes shape/text layouts. `render` method currently only accepts shape data. `cleanup` destroys shape/text layouts.
...
- **Key Structs**: `PipelineManager { shape_pipeline_layout: vk::PipelineLayout, shape_descriptor_set_layout: vk::DescriptorSetLayout, text_pipeline_layout: vk::PipelineLayout, text_descriptor_set_layout: vk::DescriptorSetLayout, descriptor_pool: vk::DescriptorPool }`. (**Updated fields**)
- **Key Methods**: `new(...) -> Self`.
- **Notes**: Provides layouts/pool to `Renderer` during initialization.

### `src/gui_framework/rendering/buffer_manager.rs`
- **Purpose**: Manages global projection UBO and per-entity Vulkan resources (vertex buffers, transform UBOs, descriptor sets) **for shapes only**. Caches pipelines/shaders. Updates vertex buffers based on `RenderCommandData.vertices_changed`. Updates descriptor sets immediately per-entity.
...
- **Notes**: Creates/updates resources based on `RenderCommandData`. Uses persistently mapped pointers. Uses `shape_pipeline_layout`. **Lacks resource removal for despawned entities.** Does not handle text vertices/pipelines/descriptors. Added type annotation for `prepared_draws`.
...
- **Key Structs**:
    ...
    - `GlyphAtlas { image: vk::Image, allocation: Option<Allocation>, image_view: vk::ImageView, sampler: vk::Sampler, extent: vk::Extent2D, format: vk::Format, current_y: u32, current_line_x: u32, current_line_max_h: u32, padding: u32, glyph_cache: HashMap<CacheKey, GlyphInfo>, scale_context: ScaleContext }`. (**Updated fields**)
- **Key Methods**: `new(...) -> Result<Self, String>`, `add_glyph(&mut self, vk_context: &VulkanContext, cache_key: CacheKey, swash_image: &swash::scale::image::Image) -> Result<&GlyphInfo, String>`, `upload_glyph_bitmap(...) -> Result<(), String>`, `cleanup(...) -> ()`. (**Updated signatures/methods**)
- **Notes**: Creates/manages Vulkan `Image`, `ImageView`, `Sampler` via `vk-mem`. `add_glyph` implements packing/upload logic. Cleaned up by `cleanup_trigger_system`. Managed via `GlyphAtlasResource`. Does not derive `Debug`.
...
- **Text Files**: `text.vert`, `text.frag`. (**New**)
- **Text Roles**: `text.vert` uses projection UBO (Set 0, Binding 0). `text.frag` uses glyph atlas sampler (Set 1, Binding 0). Input attributes: position (loc 0), UV (loc 1).
