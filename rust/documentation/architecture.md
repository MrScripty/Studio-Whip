...
4.  **Buffer Manager (`rendering/buffer_manager.rs`)**
    *   Role: **Shape resource manager.** Manages per-entity Vulkan resources (vertex buffers, transform UBOs, descriptor sets) for shapes based on ECS `RenderCommandData`. Uses shape layout/pool provided during initialization. Caches shape pipelines (including depth state) and shader modules. Updates vertex buffers based on `vertices_changed` flag (correctly handling initial spawn). Updates transform UBOs and descriptor sets every frame for existing entities. **Lacks optimization for resource removal (despawned entities).**
...
5.  **Text Handling (`rendering/font_server.rs`, `rendering/glyph_atlas.rs`, `components/text_data.rs`, `components/text_layout.rs`, `plugins/core.rs`)**
    *   Role: Manages font loading (`font_server`), text layout/shaping (`text_layout_system`), glyph caching/packing/upload (`glyph_atlas` using `rectangle-pack`), **text-specific Vulkan resource management** (`TextRenderingResources`, managed by `create_text_rendering_resources_system`), and **text rendering preparation** (`prepare_text_rendering_system`).
    *   Key Modules: `font_server.rs`, `glyph_atlas.rs`, `text_data.rs`, `text_layout.rs`. Systems in `plugins/core.rs` orchestrate layout, resource creation, and rendering preparation.
...
9.  **Framework Plugins (`plugins/`)**
    ...
    *   Key Modules: `core.rs` (Vulkan/Renderer/Text setup, text layout, **text resource management**, **text rendering preparation**, rendering system, resize, cleanup), `interaction.rs` (Input processing, hotkey loading/dispatch, window close), `movement.rs` (Default drag movement), `bindings.rs` (Default hotkey actions).
...
## Data Flow (Post Text Rendering Refactor & Depth Implementation)
...
2.  Bevy `Startup` schedule runs (ordered by `CoreSet`):
    ...
    *   `create_text_rendering_resources_system` creates `TextRenderingResources` (text vertex buffer, pipeline, atlas descriptor set).
...
3.  Bevy `Update` schedule runs (ordered by `CoreSet`):
    ...
    *   `prepare_text_rendering_system` queries `TextLayoutOutput`, generates vertices, updates text vertex buffer (via `TextRenderingResources`), and stores draw commands in `PreparedTextDrawsResource`.
...
4.  Bevy `Last` schedule runs:
    *   `rendering_system` queries ECS for shapes (`ShapeData`), collects `RenderCommandData`. Reads `PreparedTextDrawsResource`. Calls `Renderer::render`, passing shape commands and prepared text draw data.
    ...
    *   `cleanup_trigger_system` runs on `AppExit`, cleaning up framework resources (`Renderer`, `TextRenderingResources`, `GlobalProjectionUboResource`, `GlyphAtlas`, `VulkanContext`).

## Key Interactions (Post Text Rendering Refactor & Depth Implementation)
...
- **Core Plugin (Text Prep)**: `prepare_text_rendering_system` reads `TextLayoutOutput`, `GlobalTransform`; uses `TextRenderingResources`, `GlobalProjectionUboResource`, `VulkanContextResource`; writes `PreparedTextDrawsResource`.
- **Core Plugin (Rendering)**: `rendering_system` reads `ShapeData`, `GlobalTransform`, `PreparedTextDrawsResource`; calls `Renderer::render`.
- **Core Plugin (Cleanup)**: `cleanup_trigger_system` cleans up `Renderer`, `TextRenderingResources`, `GlobalProjectionUboResource`, `GlyphAtlas`, `VulkanContext`.
...
## Current Capabilities (Post Text Rendering Refactor & Depth Implementation)
...
- **ECS-Driven Resource Management**:
    ...
    - **`TextRenderingResources` holds text vertex buffer, pipeline, and atlas descriptor set.** Managed by systems in `core.rs`.
- **Resource Caching**: `BufferManager` caches shape pipelines/shaders. `GlyphAtlas` caches glyph locations/UVs.
- **Rendering Path**: Data flows from ECS (`rendering_system`) -> `Renderer` (shape prep via `BufferManager`) -> `record_command_buffers` (shape and text draw recording using prepared data). Synchronization corrected. Projection matrix uses logical window size, Y-flip, and wide depth range. **Depth testing enabled.**
...
- **Text Foundation**:
    ...
    - **Text rendering preparation system (`prepare_text_rendering_system`) generates vertices and prepares draw data (`PreparedTextDrawsResource`).**
    ...
    - **Text Vulkan resources managed by `TextRenderingResources`.**
- **Robust Shutdown**: Application exit sequence correctly cleans up Vulkan resources via `cleanup_trigger_system`.
...
- Text shaders (`text.vert`, `text.frag`) loaded by `create_text_rendering_resources_system` for text pipeline creation. `text.vert` uses Set 0 (Binding 0: projection). `text.frag` uses Set 1 (Binding 0: glyph atlas sampler).
