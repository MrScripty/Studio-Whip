...
impl Renderer {
    ...
    pub fn new(platform: &mut VulkanContext, extent: vk::Extent2D) -> Self {
        ...
        unsafe {
            ...
            platform.device.as_ref().unwrap().update_descriptor_sets(&[vk::WriteDescriptorSet {
                ...
                dst_set: pipeline_mgr.descriptor_set, // Use the global set from pipeline_manager
                dst_binding: 0, // Binding 0 for projection
                descriptor_count: 1,
        ...
        info!("[Renderer::new] Global projection UBO updated");
    ...
    pub fn resize_renderer(&mut self, vulkan_context: &mut VulkanContext, width: u32, height: u32) {
        ...
        ResizeHandler::resize(
            vulkan_context,
            // Remove renderables parameter
            self.pipeline_manager.pipeline_layout,
            self.pipeline_manager.descriptor_set, // Pass global descriptor set
            new_extent, // Pass new extent
            &mut self.buffer_manager.uniform_allocation, // Pass uniform allocation for update
        );
    ...
    pub fn render(&mut self, platform: &mut VulkanContext, _render_commands: &[RenderCommandData]) { // Prefix unused render_commands
        ...
        record_command_buffers(
            platform, // Pass mutable platform here
            &[], // Placeholder: Pass prepared_draw_data here eventually
            self.pipeline_manager.pipeline_layout,
            self.pipeline_manager.descriptor_set, // Pass global projection set
