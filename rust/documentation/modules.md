...
### Current State (Task 9 Phase 2 Complete)
...
- **ECS Core**: Application state and logic managed via Bevy ECS components (`ShapeData`, `Visibility`, `Interaction`, `Transform`, `BackgroundQuad`, `Text`, `TextLayoutOutput`, `TextRenderData`, `EditableText`, `Focus`, `CursorState`, `CursorVisual`, `TextBufferCache`, **`TextSelection`**). User input processed by plugin systems, triggering Bevy events (`EntityClicked`, `EntityDragged`, `HotkeyActionTriggered`, `YrsTextChanged`, `TextFocusChanged`). Dragging correctly updates `Transform` (**Y-axis inversion fixed**).
- **Reflection**: Core framework components, events, and resources implement `Reflect` where feasible and are registered by the plugins. `TextLayoutOutput`, `PositionedGlyph`, `TextRenderData`, `TextBufferCache`, and `TextSelection` currently do not support reflection due to containing external or Vulkan types.
...
- **Rendering Status**: Rendering triggered by `rendering_system` (in `GuiFrameworkCorePlugin`). `BufferManager` creates/updates Vulkan resources for **shapes and cursor visuals** using a **single unified pipeline and push constants for color**. **Text rendering uses per-entity Vulkan resources** (`TextRenderData` component) managed by `text_rendering_system`. `TextRenderingResources` holds the shared text pipeline and global atlas descriptor set. `GlyphAtlas` manages Vulkan texture for glyphs. `command_buffers` records draw calls for **shapes, cursor visuals, and text**. Synchronization and resize handling are corrected. **Depth testing is enabled.** **Visual output functional for shapes (with color), text, and cursor, correctly layered by Z-depth.** Optimizations needed: resource removal for despawned entities.
- **Text Handling**: Foundation laid with `Text` component, `FontServer`, `GlyphAtlas`, `SwashCache` resource, and `text_layout_system` (CPU-side layout using `cosmic-text`, triggered by `YrsTextChanged`/`Added<Text>`, **caches results in `TextBufferCache`**). Text shaders and Vulkan layouts created. **Text rendering refactored:** `text_rendering_system` creates/updates per-entity `TextRenderData` components. `rendering_system` collects data from `TextRenderData` for drawing. **Text interaction foundation implemented:** `interaction_system` performs hit detection on `EditableText` (using `get_cursor_at_position`), manages the `Focus` component, updates the `CursorState` component, **and sets collapsed `TextSelection` on click**. **Visual cursor foundation implemented:** `manage_cursor_visual_system` spawns/despawns the `CursorVisual` entity, **sets its visibility based on `TextSelection` state**; `update_cursor_transform_system` positions the cursor based on `CursorState` and layout runs.
...
- **Key Structs**:
    ...
    - `PreparedTextDrawsResource(pub Vec<PreparedTextDrawData>)` (Derives `Resource`, `Default`) - Deprecated/unused?
...
- **Key Methods (Bevy Systems)**:
    - `main() -> ()`
    - `setup_scene_ecs(...) -> ()`: Spawns initial application-specific ECS entities (background, shapes with **color**, sample text with `EditableText`), populates `YrsDocResource`. Runs after `CoreSet::CreateTextResources` and `InteractionSet::LoadHotkeys`.
...
- **Exports**: `ShapeData`, `Visibility`, `Interaction`, `Text`, `FontId`, `TextAlignment`, `EditableText`, `Focus`, **`CursorState`**, **`CursorVisual`**, **`TextSelection`**, `TextLayoutOutput`, `PositionedGlyph`, `TextRenderData`, **`TextBufferCache`**. (**Updated**)

### `src/gui_framework/components/shape_data.rs`
- **Purpose**: Defines the visual shape data for an entity (used for shapes and cursor).
- **Key Structs**: `ShapeData { vertices: Arc<Vec<Vertex>>, color: Color }` (Derives `Component`, `Debug`, `Clone`, `Reflect`, `Default`).
...
- **Key Structs**:
    ...
    - **`TextSelection { start: usize, end: usize }`** (Derives `Component`, `Debug`, `Clone`, `Copy`, `Default`, `Reflect`) - Stores selection byte offsets. (**New**)
- **Notes**: `Text` used by `text_layout_system` (core plugin). `EditableText`, `Focus`, `TextSelection` used by `interaction_system` (interaction plugin). `CursorState`, `CursorVisual` used by cursor management systems. Registered by respective plugins. Uses `bevy_color::Color`.
...
- **Key Structs**: `VulkanContext { ... shape_pipeline_layout: Option<vk::PipelineLayout>, text_pipeline_layout: Option<vk::PipelineLayout>, ... depth_image: Option<vk::Image>, depth_image_view: Option<vk::ImageView>, depth_format: Option<vk::Format>, ... debug_utils_loader: Option<debug_utils::Instance>, debug_messenger: Option<vk::DebugUtilsMessengerEXT>, ... }`.
...
- **Key Structs**:
    ...
    - `YrsTextChanged { entity: Entity }` (Derives `Event`, `Reflect`) - Sent when Yrs data changes.
    - `TextFocusChanged { entity: Option<Entity> }` (Derives `Event`, `Reflect`) - Sent when text focus changes.
- **Notes**: Written by systems in `GuiFrameworkInteractionPlugin` (and potentially Yrs update systems later), read by other plugin/app systems. Registered by `GuiFrameworkInteractionPlugin`.

### `src/gui_framework/interaction/mod.rs`
- **Purpose**: Declares interaction-related submodules. (**Updated**)
- **Modules**: `hotkeys.rs`, `utils.rs`. (**Updated**)
...
### `src/gui_framework/interaction/utils.rs` (**New**)
...
- **Purpose**: Plugin for core Vulkan setup, rendering (shapes, cursor, text), text foundation (setup, layout, atlas, caching), **visual cursor management (including visibility based on selection)**, **text resource management**, resize handling, and cleanup. (**Updated**)
- **Key Structs**: `GuiFrameworkCorePlugin`, `CoreSet` (enum SystemSet: `SetupVulkan`, `CreateRenderer`, `CreateGlyphAtlas`, `CreateFontServer`, `CreateSwashCache`, `CreateGlobalUbo`, `CreateTextResources`, **`UpdateCursorTransform`**, **`ManageCursorVisual`**, `TextLayout`, `TextRendering`, `HandleResize`, `Render`, `Cleanup`).
- **Key Methods (Bevy Systems)**: `setup_vulkan_system() -> ()`, `create_renderer_system() -> ()`, `create_glyph_atlas_system() -> ()`, `create_font_server_system() -> ()`, `create_swash_cache_system() -> ()`, `create_global_ubo_system() -> ()`, `create_text_rendering_resources_system() -> ()`, `handle_resize_system(...) -> ()`, `text_layout_system(...) -> ()`, `text_rendering_system(...) -> ()`, **`manage_cursor_visual_system(...) -> ()`** (reads `TextSelection`, sets `Visibility`), **`update_cursor_transform_system(...) -> ()`**, `rendering_system(...) -> ()`, `cleanup_trigger_system(world: &mut World) -> ()`. (**Updated**)
- **Notes**: Adds systems to `Startup`, `Update`, `Last` schedules. Configures `CoreSet` ordering. Registers core types (including `CursorState`, `CursorVisual`). Inserts framework resources. `text_layout_system` performs CPU layout, reads from `YrsDocResource`, writes `TextBufferCache`. `text_rendering_system` creates/updates per-entity `TextRenderData`. `manage_cursor_visual_system` spawns/despawns `CursorVisual` based on `Focus`, **sets visibility based on `TextSelection`**. `update_cursor_transform_system` positions `CursorVisual` based on `CursorState` and `TextBufferCache`. `rendering_system` prepares shape/cursor/text data and calls `Renderer::render`. `cleanup_trigger_system` cleans per-entity resources and shared Vulkan resources.

### `src/gui_framework/plugins/interaction.rs`
- **Purpose**: Plugin for input processing (mouse, keyboard), hotkey loading/dispatch, window close requests, and **ECS-based text focus, cursor state, and selection state management**. (**Updated**)
- **Key Structs**:
    - `GuiFrameworkInteractionPlugin`, `InteractionSet` (enum SystemSet).
    - `MouseContextType` (enum: `Idle`, `DraggingShape`, `TextInteraction`) - Local enum. (**New**)
    - `MouseContext { context: MouseContextType }` (Derives `Resource`, `Default`) - Local resource. (**New**)
- **Key Methods (Bevy Systems)**:
    - `load_hotkeys_system(mut commands: Commands) -> ()`
    - `interaction_system(...) -> ()`: Processes mouse clicks/drags. **Sets `MouseContext`**. Performs hit detection on text using `get_cursor_at_position`. Manages `Focus` component. **Updates `CursorState` and adds/updates `TextSelection` (collapsed) on click.** Sends events. (**Updated**)
    - `hotkey_system(...) -> ()`
    - `handle_close_request(...) -> ()`
- **Notes**: Adds systems to `Startup`, `Update` schedules. **Configures `InteractionSet` to run before `CoreSet::ManageCursorVisual`**. Registers `Interaction`, `HotkeyResource`, `HotkeyConfig`, `EditableText`, `Focus`, **`TextSelection`**, interaction events. Inserts `HotkeyResource`. **Initializes local `MouseContext` resource.**
...
- **Notes**: Called by `render_engine.rs`. Binds shape resources per entity, pushes color constant. Iterates through `PreparedTextDrawData`, binding text pipeline, per-entity vertex buffer, per-entity descriptor set 0, and global atlas descriptor set 1 for each text draw.

### `src/gui_framework/rendering/render_engine.rs`
- **Purpose**: Orchestrates custom Vulkan rendering per frame. Manages `BufferManager` (shapes/cursor) and sync objects. Handles resize via `ResizeHandler`. Integrates shape, cursor, and text rendering paths using prepared data. Manages internal locking for Vulkan context access.
- **Key Structs**: `Renderer { buffer_manager: BufferManager, descriptor_pool: vk::DescriptorPool, descriptor_set_layout: vk::DescriptorSetLayout, text_descriptor_set_layout: vk::DescriptorSetLayout }`.
...
- **Key Structs**: `EntityRenderResources`, `PipelineCacheKey`, `BufferManager`.
- **Key Methods**: `new(_platform: &mut VulkanContext, per_entity_layout: vk::DescriptorSetLayout, descriptor_pool: vk::DescriptorPool) -> Self`, `prepare_frame_resources(platform: &mut VulkanContext, render_commands: &[RenderCommandData], global_ubo_res: &GlobalProjectionUboResource) -> Vec<PreparedDrawData>`, `cleanup(platform: &mut VulkanContext) -> ()`.
