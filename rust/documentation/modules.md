...
### Current State (Post Text Rendering Implementation & Fixes)
...
- **Rendering Status**: Rendering triggered by `rendering_system` (in `GuiFrameworkCorePlugin`). `BufferManager` creates/updates Vulkan resources for **shapes**. `Renderer` manages dynamic vertex buffer, descriptor set, and pipeline for **text**. `GlyphAtlas` manages Vulkan texture for glyphs. `command_buffers` records draw calls for **shapes and text**. Synchronization and resize handling are corrected. **Visual output functional for shapes and baseline-aligned static text.** Optimizations needed: resource removal for despawned entities, text rendering efficiency/encapsulation, text descriptor set binding (Set 0).
- **Text Handling**: Foundation laid with `Text` component, `FontServer` (loading system fonts), `GlyphAtlas` (Vulkan texture setup, packing via `rectangle-pack`, rasterization data input, upload), `SwashCache` resource, and `text_layout_system` (CPU-side layout using `cosmic-text`, calculates baseline-aligned vertices, calls `add_glyph`). Text shaders and Vulkan layouts created. **Basic text rendering pipeline implemented:** `rendering_system` generates vertices (with flipped UVs), `Renderer` manages buffer/pipeline/descriptor set, `command_buffers` draws text.
...
- **Key Structs**:
    ...
    - `TextRenderCommandData { vertex_buffer_offset: u32, vertex_count: u32 }` (Derives `Debug`, `Clone`)
...
- **Key Methods (Bevy Systems)**:
    - `main() -> ()`
    - `setup_scene_ecs(...) -> ()`: Spawns initial application-specific ECS entities (background, shapes, sample text). Runs after `CoreSet::CreateRenderer`.
...
- **Notes**: Adds systems to `Startup`, `Update`, `Last` schedules. Configures `CoreSet` ordering. Registers core types. Inserts `RendererResource`, `GlyphAtlasResource`, `FontServerResource`, `SwashCacheResource`. `text_layout_system` performs CPU layout (using `cosmic-text`), calculates baseline-aligned vertices, and triggers glyph upload. `rendering_system` prepares shape data, **generates text vertices (with flipped UVs)**, and calls `Renderer::render`. `cleanup_trigger_system` cleans Vulkan resources.
...
- **Key Methods**: `record_command_buffers(platform: &mut VulkanContext, prepared_shape_draws: &[PreparedDrawData], text_vertex_buffer: vk::Buffer, glyph_atlas_descriptor_set: vk::DescriptorSet, text_command: Option<&TextRenderCommandData>, extent: vk::Extent2D, text_pipeline: vk::Pipeline) -> ()`.
...
- **Key Structs**:
    ...
    - `GlyphAtlas { image: vk::Image, allocation: Option<Allocation>, image_view: vk::ImageView, sampler: vk::Sampler, extent: vk::Extent2D, format: vk::Format, target_bins: BTreeMap<u32, TargetBin>, padding: u32, glyph_cache: HashMap<CacheKey, GlyphInfo>, scale_context: ScaleContext }`. (**Updated fields**)
